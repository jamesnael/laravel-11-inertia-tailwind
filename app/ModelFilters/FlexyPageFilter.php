...
class FlexyPageFilter extends ModelFilter
{
    /**
    ...
    */
    ...
    {
        $this->country($value)->agreement($value, true);
    }
    ...
    public function agreement($value, $orWhere = false)
    {
        if ($orWhere) {
            return $this->orWhere('title', 'LIKE', '%' . $value . '%')->where('is_agreement', true);
        }

        return $this->where('title', 'LIKE', '%' . $value . '%')->where('is_agreement', true);
    }
