<template>
    ...
    <admin-layout>
        ...
        <datatables
            ref="datatables"
            class="pt-4"
            :route-uri="route('user.product.table')"
        ...
        >
            ...
            <template #after.reload-button>
                <Link
                    v-if="
                        hasAccess(
                            'module.user.product.create',
                            $page.props.currentUser.jabatan.hak_akses
                        )
                    "
                    :href="route('user.product.create')"
                    class="p-3 text-center shadow-md rounded-md font-semibold text-white bg-blue-500 focus:outline-none focus:ring-4 focus:ring-blue-300"
                >
                    <div class="flex content-center items-center">
                        <svg
                            ...
                            height="16"
                            fill="currentColor"
                            class="w-4 h-4 md:w-3 md:h-3 stroke-current"
                            viewBox="0 0 16 16"
                        >
                            <path
            ...
            <template #grid.content.body.action="{ row }">
                <div class="flex flex-row justify-center space-x-4">
                    ...
                    <!-- <button
                        v-if="
                            hasAccess(
                                'module.user.product.delete',
                                $page.props.currentUser.jabatan.hak_akses
                            )
                        "
                        type="button"
                        class="appearance-none outline-none focus:border-transparent focus:outline-none bg-transparent"
                        @click.prevent="confirmDeleteRow(row)"
                    >
                        <svg
                            xmlns="http://www.w3.org/2000/svg"
                            width="16"
                            height="16"
                            fill="currentColor"
                            class="w-4 h-4 stroke-current text-red-600"
                            viewBox="0 0 16 16"
                        >
                            <path
            ...
            <template #table.cell.content.approvalStatus="{ row }">
                <span :class="{
                    'text-white font-bold py-2 px-4 rounded-lg' : true,
                    'bg-yellow-400' : row.approval_status == 'Waiting Approval',
                    'bg-red-400' : row.approval_status == 'Draft',
                    'bg-green-400' : row.approval_status == 'Approved',
                }">
                    {{ row.approval_status }}
                </span>
            </template>
            <template #table.cell.content.productStatus="{ row }">
                <span
                v-if="row.status" :class="{
                    'text-white font-bold py-2 px-4 rounded-lg' : true,
                    'bg-red-400' : row.status == 'Draft',
                    'bg-yellow-400' : row.status == 'Unpublished',
                    'bg-green-400' : row.status == 'Published',
                }">
                    {{ row.status }}
                </span>
            ...
            <template #table.cell.content.productCategory="{ row }">
                <div v-for="(el, idx) in row.product_category" v-if="row.product_category">
                    <span > {{ el.category.nama }}</span>
                    <span v-if="idx != Object.keys(row.product_category).length - 1">, </span>
                </div>
                <div else>
            ...
            <template #table.cell.content.productCountry="{ row }">
                <div v-for="(el, idx) in row.product_country" v-if="row.product_country">
                    <span > {{ el.country.country }}</span>
                    <span v-if="idx != Object.keys(row.product_country).length - 1">, </span>
                </div>  
                <div else>
            ...
            <template #table.cell.content.productCompany="{ row }">
                <span> {{ row.product_company ? row.product_company.title : null }}</span>
            </template>

            <template #table.cell.content.date="{ row }">
        ...
        <dialog-modal :show="showModal" @close="showModal = false">
            <div class="flex flex-col space-y-6 p-10 items-center">
                ...
                <p class="text-center md:text-lg">
                    Are you sure to delete this product? products that have been deleted cannot be restored.
                </p>
...
export default {
    ...
    setup() {
        ...
        const columns = ref([
            ...
            {
                uniqid: "approvalStatus",
                label: "Approval Status",
                field: "approval_status",
                sortable: false,
                sortOrder: "asc",
                align: "center",
                classes: "px-4 py-2 md:py-4 text-left md:text-center w-52",
                headerClass: "text-center p-4",
            },
            {
                uniqid: "productStatus",
        ...
        function deleteRow() {
            form.delete(
                route("user.product.destroy", {
                    product: form.value.slug,
                }),
                {
                    onSuccess: () => {
                        form.reset();
