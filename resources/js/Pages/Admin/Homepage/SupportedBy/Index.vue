<template>
    ...
    <admin-layout>
        ...
        <datatables
            ref="datatables"
            class="pt-4"
            :route-uri="route('admin.homepage.supported-by.table')"
            ...
            search-label="Search Supported By"
        >
            ...
            <template #after.reload-button>
                <Link
                    v-if="
                        hasAccess(
                            'module.homepage.supported-by.create',
                            $page.props.currentUser.jabatan.hak_akses
                        )
                    "
                    :href="route('admin.homepage.supported-by.create')"
            ...
            <template #grid.content.body.action="{ row }">
                <div class="flex flex-row justify-center space-x-4">
                    <Link
                        v-if="
                            hasAccess(
                                'module.homepage.supported-by.edit',
                                $page.props.currentUser.jabatan.hak_akses
                            )
                        "
                        :href="
                            route('admin.homepage.supported-by.edit', {
                                supported_by: row.slug,
                            })
                        "
                    >
                        <svg
            ...
            <template #table.cell.content.action="{ row }">
                <div class="flex flex-row justify-center space-x-4">
                    ...
                    <button
                        v-if="
                            hasAccess(
                                'module.homepage.supported-by.delete',
                                $page.props.currentUser.jabatan.hak_akses
                            )
                        "
                        type="button"
            ...
            <template #table.cell.content.image="{ row }">
                <div align="center">
                    <img :src="row.image" width="90" />
                </div>
            </template>
            <template #table.cell.content.status="{ row }">
                <span v-if="row.status" class="bg-blue-500 text-white font-bold py-2 px-4 rounded-lg">Publish</span>
                <span v-else class="bg-red-500 text-white font-bold py-2 px-4 rounded-lg">Unpublish</span>
            </template>
        </datatables>
        ...
        <dialog-modal :show="showModal" @close="showModal = false">
            <div class="flex flex-col space-y-6 p-10 items-center">
                ...
                <p class="text-center md:text-lg">
                    Are you sure to delete this Supported By? logos that have been deleted cannot be restored.
                </p>
...
export default {
    ...
    setup() {
        ...
        function deleteRow() {
            form.delete(
                route("admin.homepage.supported-by.destroy", {
                    supported_by: form.value.slug,
                }),
                {
                    onSuccess: () => {
                        form.reset();
