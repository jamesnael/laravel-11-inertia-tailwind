<template>
    ...
    <admin-layout>
        ...
        <datatables
            ref="datatables"
            class="pt-4"
            :route-uri="route('admin.homepage.sliding-banner.table')"
        ...
        >
            ...
            <template #after.reload-button>
                <Link
                    v-if="
                        hasAccess(
                            'module.homepage.sliding-banner.create',
                            $page.props.currentUser.jabatan.hak_akses
                        )
                    "
                    :href="route('admin.homepage.sliding-banner.create')"
                ...
                <Link
                    v-if="
                        hasAccess(
                            'module.homepage.sliding-banner.edit',
                            $page.props.currentUser.jabatan.hak_akses
                        )
                    "
                    :href="route('admin.homepage.sliding-banner.position')"
                    class="p-3 text-center shadow-md rounded-md font-semibold text-white bg-blue-500 focus:outline-none focus:ring-4 focus:ring-blue-300"
                >
                    <div class="flex content-center items-center">
                        <span class="hidden md:inline-flex ml-2"
                            >Change Secondary Banner Position</span
                        >
                    </div>
                </Link>
            </template>
            <template #grid.content.body.action="{ row }">
                <div class="flex flex-row justify-center space-x-4">
                    <Link
                        ...
                        "
                        :href="
                            route('admin.homepage.sliding-banner.edit', {
                                banner: row.slug,
                            })
                        "
                    >
                    ...
                    <button
                        v-if="
                            hasAccess(
                                'module.homepage.sliding-banner.delete',
                                $page.props.currentUser.jabatan.hak_akses
                            )
                        "
                        type="button"
            ...
            <template #table.cell.content.tipe="{ row }">
        ...
        <dialog-modal :show="showModal" @close="showModal = false">
            <div class="flex flex-col space-y-6 p-10 items-center">
                ...
                <p class="text-center md:text-lg">
                    Are you sure to delete this Sliding Banner? Sliding Banners
                    that have been deleted cannot be restored.
                </p>
...
export default {
    ...
    setup() {
        ...
        function deleteRow() {
            form.delete(
                route("admin.homepage.sliding-banner.destroy", {
                    banner: form.value.slug,
                }),
                {
                    onSuccess: () => {
                        form.reset();
