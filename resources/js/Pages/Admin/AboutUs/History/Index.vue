<template>
  ...
  <admin-layout>
    ...
    <template #page-title>
      <h2 class="text-dark-7 text-lg font-medium mr-auto">History</h2>
    </template>

    <datatables
      ref="datatables"
      class="pt-4"  
      :route-uri="route('admin.about-us.history.table')"
      ...
      <template #label.no-result>
        <div class="py-2">No record match the search criteria.</div>
      </template>
      ...
      <template #after.reload-button>
        <Link v-if="hasAccess('module.about-us.history.create', $page.props.currentUser.jabatan.hak_akses)" :href="route('admin.about-us.history.create')" class="p-3 text-center shadow-md rounded-md font-semibold text-white bg-blue-500 focus:outline-none focus:ring-4 focus:ring-blue-300">
          <div class="flex content-center items-center">
            ...
            <span class="hidden md:inline-flex ml-2">Create History</span>
          </div>
        </Link>
      </template>
      <template #table.cell.content.action="{ row }">
        <div class="flex flex-row justify-center space-x-4">
          <Link v-if="hasAccess('module.about-us.history.edit', $page.props.currentUser.jabatan.hak_akses)" :href="route('admin.about-us.history.edit', {history: row.slug})">
          ...
          <button v-if="hasAccess('module.about-us.history.delete', $page.props.currentUser.jabatan.hak_akses)" type="button" class="appearance-none outline-none focus:border-transparent focus:outline-none bg-transparent" @click.prevent="confirmDeleteRow(row)">
    ...
    <dialog-modal :show="showModal" @close="showModal = false">
      <div class="flex flex-col space-y-6 p-10 items-center">
        ...
        <p class="text-center md:text-lg">
          Are you sure to delete this History? History that have been deleted cannot be restored.
        </p>
...
<script>
  ...
  export default {
    ...
    setup() {
      ...
      function deleteRow() {
        form.delete(route('admin.about-us.history.destroy', { history: form.value.slug }), {
          onSuccess: () => {
            form.reset()
