<template>
  ...
  <admin-layout>
    ...
    <datatables
      ref="datatables"
      class="pt-4"
      :route-uri="route('admin.settings.karyawan.table')"
      ...
      <template #after.reload-button>
        <Link v-if="hasAccess('module.settings.karyawan.create', $page.props.currentUser.jabatan.hak_akses)" :href="route('admin.settings.karyawan.create')" class="p-3 text-center shadow-md rounded-md font-semibold text-white bg-blue-500 focus:outline-none focus:ring-4 focus:ring-blue-300">
      ...
      <template #grid.content.body.action="{ row }">
        <div class="flex flex-row justify-center space-x-4">
          <Link v-if="hasAccess('module.settings.karyawan.edit', $page.props.currentUser.jabatan.hak_akses)" :href="route('admin.settings.karyawan.edit', {karyawan: row.slug})">
          ...
          <button v-if="hasAccess('module.settings.karyawan.delete', $page.props.currentUser.jabatan.hak_akses)" type="button" class="appearance-none outline-none focus:border-transparent focus:outline-none bg-transparent" @click.prevent="confirmDeleteRow(row)">
    ...
    <dialog-modal :show="showModal" @close="showModal = false">
      <div class="flex flex-col space-y-6 p-10 items-center">
        ...
        <p class="text-center md:text-lg">
          Are you sure to delete this User? User that have been deleted cannot be restored.
        </p>
...
<script>
  ...
  export default {
    ...
    setup() {
      ...
      function deleteRow() {
        form.delete(route('admin.settings.karyawan.destroy', { karyawan: form.value.slug }), {
          onSuccess: () => {
            form.reset()
