    ...
    <div class="flex flex-col space-y-4">
      <div class="flex flex-col md:flex-row space-y-4 md:space-y-0 md:space-x-8">
        ...
        <div class="flex-1 flex flex-col space-y-2">
          ...
          <select-search
            clearable
            :class="{ 'rounded-md focus:ring-1 ring-indigo-500 placeholder-gray-500 text-black disabled:cursor-not-allowed disabled:bg-gray-100': true, 'border-red-400': form.errors.id_jabatan }"
            v-model="form.id_jabatan"
            :disabled="form.processing"
            :options="jabatan">
          </select-search>
          <span v-if="form.errors.id_jabatan" class="text-red-400 italic">{{ form.errors.id_jabatan }}</span>
      ...
      <div class="flex flex-col md:flex-row space-y-4 md:space-y-0 md:space-x-8">
        <div class="flex-1 flex flex-col space-y-2">
          ...
          <span v-if="karyawan" class="text-black italic text-xs">Leave it blank if you don't want to change the user password</span>
      ...
      <div class="flex flex-row justify-end space-x-4">
        <Link class="py-3 px-6 text-center shadow-md rounded-md font-semibold text-white bg-gray-400 focus:outline-none focus:ring-4 focus:ring-gray-300 disabled:cursor-not-allowed" :href="route('admin.settings.karyawan.index')" :disabled="form.processing">
          Back
        </Link>
    ...
    setup (props) {
      ...
      onMounted(() => {
        if (props.karyawan) {
          ...
          form.id_jabatan = _.toString(props.karyawan.id_jabatan)
      ...
      const showJabatanLabel = computed(() => {
          if (form.id_jabatan) {
              let data = _.filter(props.jabatan, jabatan => jabatan.value == form.id_jabatan);
