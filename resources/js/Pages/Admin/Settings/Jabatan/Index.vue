<template>
  ...
  <admin-layout>
    ...
    <datatables
      ref="datatables"
      class="pt-4"
      :route-uri="route('admin.settings.jabatan.table')"
      ...
      search-label="Search Group User" >
      ...
      <template #after.reload-button>
        <Link v-if="hasAccess('module.settings.jabatan.create', $page.props.currentUser.jabatan.hak_akses)" :href="route('admin.settings.jabatan.create')" class="p-3 text-center shadow-md rounded-md font-semibold text-white bg-blue-500 focus:outline-none focus:ring-4 focus:ring-blue-300">
      ...
      <template #grid.content.body.action="{ row }">
        <div class="flex flex-row justify-center space-x-4">
          <Link v-if="hasAccess('module.settings.jabatan.edit', $page.props.currentUser.jabatan.hak_akses)" :href="route('admin.settings.jabatan.edit', {jabatan: row.slug})">
          ...
          <button v-if="hasAccess('module.settings.jabatan.delete', $page.props.currentUser.jabatan.hak_akses)" type="button" class="appearance-none outline-none focus:border-transparent focus:outline-none bg-transparent" @click.prevent="confirmDeleteRow(row)">
      ...
      <template #table.cell.content.action="{ row }">
        <div class="flex flex-row justify-center space-x-4">
          ...
          <template v-if="row.nama_jabatan != 'Private Sector'">
            <button v-if="hasAccess('module.settings.jabatan.delete', $page.props.currentUser.jabatan.hak_akses)" type="button" class="appearance-none outline-none focus:border-transparent focus:outline-none bg-transparent" @click.prevent="confirmDeleteRow(row)">
              <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="w-4 h-4 stroke-current text-red-600" viewBox="0 0 16 16">
                <path d="M5.5 5.5A.5.5 0 0 1 6 6v6a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5zm2.5 0a.5.5 0 0 1 .5.5v6a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5zm3 .5a.5.5 0 0 0-1 0v6a.5.5 0 0 0 1 0V6z"/>
                <path fill-rule="evenodd" d="M14.5 3a1 1 0 0 1-1 1H13v9a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V4h-.5a1 1 0 0 1-1-1V2a1 1 0 0 1 1-1H6a1 1 0 0 1 1-1h2a1 1 0 0 1 1 1h3.5a1 1 0 0 1 1 1v1zM4.118 4 4 4.059V13a1 1 0 0 0 1 1h6a1 1 0 0 0 1-1V4.059L11.882 4H4.118zM2.5 3V2h11v1h-11z"/>
    ...
    <dialog-modal :show="showModal" @close="showModal = false">
      <div class="flex flex-col space-y-6 p-10 items-center">
        ...
        <p class="text-center md:text-lg">
          Are you sure to delete this Group User? Group Users that have been deleted cannot be restored.
        </p>
...
<script>
  ...
  export default {
    ...
    setup() {
      ...
      function deleteRow() {
        form.delete(route('admin.settings.jabatan.destroy', { jabatan: form.value.slug }), {
          onSuccess: () => {
            form.reset()
