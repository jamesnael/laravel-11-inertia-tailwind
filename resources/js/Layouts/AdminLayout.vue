<template>
  ...
  <div class="flex min-h-screen-95">
      <!-- BEGIN: Side Menu -->
      <nav class="pr-5 h-full overflow-x-hidden hidden md:block flex-flex-col fixed bg-dark-3" style="width: 270px;">
        <div class="h-full flex flex-col">
          <a href="" class="flex items-center pl-5 pt-6">
            <!-- <logo-image class="w-6 h-6" /> -->
            <span class="hidden xl:block text-white text-md ml-3">
              <span class="font-medium">KEMENHAM</span>
            </span>
          </a>
          <div class="w-full h-px z-10 relative bg-dark-3 my-6"></div>
          <div class="overflow-y-hidden hover:overflow-y-auto pl-3" style="height: calc(103vh - 210px)">
            <main-menu />
          </div>
          <user-menu
            hide-namecard
            ...
            <hr class="mt-5 ml-3" />
            <div class="py-2 mt-2 pl-4 border-b border-dark-3 flex items-center">
              <!-- <img :src="'https://ui-avatars.com/api/?background=0D8ABC&color=fff&name=' + $page.props.currentUser.nama_lengkap" :alt="$page.props.currentUser.nama_lengkap" class="rounded-full h-8 w-8 object-cover"> -->
              <svg xmlns="http://www.w3.org/2000/svg" width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5.52 19c.64-2.2 1.84-3 3.22-3h6.52c1.38 0 2.58.8 3.22 3"/><circle cx="12" cy="10" r="3"/><circle cx="12" cy="12" r="10"/></svg>
