    ...
    <ul class="rounded-md bg-dark-2">
        <li v-for="submenu in menu.children">
          ...
          <div v-if="!submenu.children">
            <Link v-if="hasAccess(submenu.access, $page.props.currentUser.jabatan.hak_akses)" :href="submenu.route === '#' ? '#' : route(submenu.route)" :class="{ 'ml-3 h-12 flex items-center pl-5 mb-1 relative rounded-full': true, 'text-white': submenu.active, 'text-gray-300': !submenu.active }">
          ...
          <div v-else>
            <ChildSubMenu :submenu="submenu"></ChildSubMenu>
              <!-- <a v-if="hasAccess(submenu.access, $page.props.currentUser.jabatan.hak_akses)" href="javascript:;" :class="{ 'h-12 flex items-center pl-5 mb-1 relative rounded-full': true, 'text-white': submenu.active, 'text-gray-300 hover:bg-dark-2 hover:text-gray-200': !submenu.active }" @click.prevent="openSubMenu = !openSubMenu">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="w-2 h-2" viewBox="0 0 16 16">
                  <circle cx="8" cy="8" r="8"/>
                </svg>
                <div class="w-full ml-3 flex items-center">
              ...
              <TransitionSlide :show="openSubMenu">
                <ul class="rounded-md bg-dark-2 ml-3">
...
<script>
  ...
  import ChildSubMenu from "@/Layouts/ChildSubMenu.vue";
  ...
  export default {
    ...
    setup(props) {
      ...
      onMounted(() => { openMenu.value = props.menu.open,openSubMenu.value = props.menu.open })
